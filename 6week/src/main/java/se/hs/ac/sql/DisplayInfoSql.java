...
public class DisplayInfoSql {
    ...
    public static final String SELECT_DISPLAY_INFOS_LIMIT_FOUR = "select p.id, p.category_id, d.id as display_info_id ,c.name, p.description, p.content, p.event, d.opening_hours, d.place_name, d.place_lot, d.place_street, d.tel, d.homepage, d.email, d.create_date, d.modify_date, pi.file_id\n" +
            "from product as p, display_info as d, category as c, file_info as f, product_image as pi\n" +
            "where p.id = d.id and p.category_id = c.id and f.id=pi.file_id and pi.product_id = p.id and pi.type=:type and p.category_id= :category_id limit :start,4 ";
    public static final String SELECT_DISPLAY_INFOS_BY_ID = "select p.id, p.category_id, dii.display_info_id , c.name, p.description, p.content, p.event, d.opening_hours, d.place_name, d.place_lot, d.place_street, d.tel, d.homepage, d.email, d.create_date, d.modify_date, pi.file_id\n" +
    ...
    public static final String SELECT_DISPLAY_INFOS_ALL = "select p.id, p.category_id, d.id as display_info_id ,c.name, p.description, p.content, p.event, d.opening_hours, d.place_name, d.place_lot, d.place_street, d.tel, d.homepage, d.email, d.create_date, d.modify_date, pi.file_id\n" +
            "from product as p, display_info as d, category as c, file_info as f, product_image as pi\n" +
            "where p.id = d.id and p.category_id = c.id and f.id=pi.file_id and pi.product_id = p.id and pi.type=:type";
